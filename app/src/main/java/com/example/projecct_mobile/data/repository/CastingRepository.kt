...
import com.example.projecct_mobile.data.model.AcceptCandidateWithInterviewRequest
import com.example.projecct_mobile.data.model.AcceptCandidateWithInterviewResponse
...
class CastingRepository {
        ...
        return try {
            ...
            try {
                ...
                if (videoUri != null) {
                    ...
                    tempFile = File.createTempFile("audition_video", ".mp4", context.cacheDir)
                ...
                // Si feedback IA fourni, créer le RequestBody (texte simple pour compatibilité multipart)
                if (aiFeedback != null) {
                    aiFeedbackBody = aiFeedback.toRequestBody("text/plain".toMediaTypeOrNull())
                }
                ...
                if (response.isSuccessful) {
                    ...
                    val exception = when (errorCode) {
                        401 -> ApiException.UnauthorizedException("Vous devez être connecté pour postuler")
                        403 -> ApiException.ForbiddenException(errorMessage ?: "Vous ne pouvez pas postuler à ce casting")
                        404 -> ApiException.NotFoundException("Casting non trouvé")
                        409 -> ApiException.ConflictException(errorMessage ?: "Vous avez déjà postulé à ce casting")
                        400 -> ApiException.BadRequestException(errorMessage ?: "Erreur lors de la candidature")
                        in 500..599 -> ApiException.ServerException("Erreur serveur: ${errorMessage ?: errorBody}")
    ...
    /**
     * Accepter un candidat avec 3 dates d'interview proposées (Recruteur/Admin)
     */
    suspend fun acceptCandidateWithInterview(
        castingId: String,
        acteurId: String,
        dates: List<InterviewDateOption>
    ): Result<AcceptCandidateWithInterviewResponse> {
        return try {
            // Valider que 3 dates sont bien fournies
            if (dates.size < 3) {
                return Result.failure(
                    ApiException.BadRequestException("Veuillez proposer 3 dates d'interview.")
                )
            }
            
            val request = AcceptCandidateWithInterviewRequest(
                proposedDates = dates
            )
            
            val response = castingService.acceptCandidateWithInterview(
                id = castingId,
                acteurId = acteurId,
                request = request
            )
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                val errorCode = response.code()
                val errorMessage = when (errorCode) {
                    400 -> "Dates invalides ou manquantes"
                    403 -> "Vous n'êtes pas autorisé à accepter ce candidat"
                    404 -> "Casting ou candidat introuvable"
                    409 -> "Une interview existe déjà pour ce candidat"
                    else -> "Erreur ${errorCode}: ${response.message()}"
                }
                Result.failure(
                    when (errorCode) {
                        400 -> ApiException.BadRequestException(errorMessage)
                        403 -> ApiException.ForbiddenException(errorMessage)
                        404 -> ApiException.NotFoundException(errorMessage)
                        409 -> ApiException.ConflictException(errorMessage)
                        else -> ApiException.UnknownException(errorMessage)
                    }
                )
            }
